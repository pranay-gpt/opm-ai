...
// 4. The inner branch must check both `results.plots` truthy AND non-empty
//    so that `{}` and `undefined` both fall through to the placeholder.
const innerCheck = /results\.plots\s*&&\s*Object\.keys\(results\.plots\)\.length\s*>\s*0/;
assert.ok(
  innerCheck.test(source),
  'ResultsViewer Plots tab inner branch must require BOTH `results.plots` truthy AND non-empty before rendering cards. ' +
    'This is the F1.6 invariant — missing OR empty plots should hit the empty-state branch.'
);
...
console.log('OK: ResultsViewer F1.6 + F8.3 + F8.9 source-grep guards verified');
